...
import { z } from "zod";
import { Branch, Commit, GitExec, changesSchema } from "./git";
...
export const appRouter = router({
  ...
  commitCallback: publicProcedure
    ...
    .query(
      async ({
        input: { orgName, repoName, branchName, commit: commitFromClient },
        ctx: { db },
      }): Promise<
        | {
            commitOid: string;
      ...
      > => {
        ...
        if (currentCommit.oid === commitFromClient.oid) {
          return { commitOid: currentCommit.oid, changes: [] };
        }
        let foundCommit: Commit | undefined;
        await currentCommit.walkParents(async (commit) => {
          if (commit.oid === commitFromClient.oid) {
            foundCommit = commit;
            return true;
          }
          return false;
        ...
        if (foundCommit) {
          return {
            commitOid: foundCommit.oid,
            changes: await branch.changesSince(foundCommit.oid),
          };
        }
      }
    ),
